<?php 
	...
	$sql_getAnnouncement = "SELECT * FROM users, profile WHERE users.type = 'student' AND profile.user_id = users.id ORDER BY users.lname";
  	...
  	if (mysqli_num_rows($getAnnouncement_query) == 0) {
		?>
  		<div class="loaded">
  			<?php 
  			array_push($errors, 'Sorry, no students available.');
