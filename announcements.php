<?php 
	...
	$sql_getAnnouncement = "SELECT *, posts.id AS post_id FROM posts, users WHERE posts.post_type = 'Announcement' AND users.id = '$id' ORDER BY posts.id DESC";
  	$getAnnouncement_query = mysqli_query($db, $sql_getAnnouncement);

  	if (mysqli_num_rows($getAnnouncement_query) == 0) {
		array_push($errors, 'Sorry, no announcements posted by the admin.');
		include "server/errors.php";
	  	...
	  	while ($rows = mysqli_fetch_array($getAnnouncement_query)) {
	  		...
	  		echo "<div id=". $post_id ." class='announcement'>
	  		<div class='top-info'>
